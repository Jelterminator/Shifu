...
export function ProjectCard({
  ...
  const handleToggleSubtask = async (task: Task): Promise<void> => {
    try {
        ...
        <View style={styles.metaRow}>
          {project.deadline && (
            <Text style={[styles.metaText, { color: colors.textSecondary }]}>
              ...
              {Math.ceil(daysUntilDeadline || 0)} days)
            </Text>
          )}
          <Text style={[styles.metaText, { color: colors.textSecondary }]}>
            ⏱️ {Math.round(totalEffort / 60)}h total · Need{' '}
            {Math.round((minutesPerDay / 60) * 10) / 10}h/day
          </Text>
        </View>
          ...
          <View style={[styles.subtasksContainer, { backgroundColor: colors.background }]}>
            ...
            {subtasks.slice(0, 5).map((t, index) => (
              <View key={t.id} style={styles.subtaskRow}>
                ...
                >
                  <Text style={[styles.subtaskStatus, { color: colors.text }]}>
                    {t.isCompleted ? '☑' : '☐'}
                  </Text>
                </TouchableOpacity>
              </View>
