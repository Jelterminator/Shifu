...
export class SchedulerAI {
  ...
  private async scheduleDay(userId: string, date: Date, fromNow: boolean, externalScheduledIds: Set<string>): Promise<void> {
    console.log(`🤖 SchedulerAI: Scheduling day ${date.toISOString().slice(0,10)} (fromNow=${fromNow})`);
    ...
    const activePhases = phaseManager.getPhasesForGregorianDate(date)
    ...
    const baseBusySlots = [
        ...
        ...anchors.map(a => ({ start: a.startTime, end: a.startTime.getTime() + a.durationMinutes * 60000 })),
        ...sleepConstraints
    ].map(slot => ({ 
    ...
    const isAllowed = (item: any, context: 'work' | 'outside') => {
        const itemKeywords: string[] = item.selectedKeywords || [];
        const lists = useListStore.getState().lists;
        
        // Find matching lists
        const matchingLists = lists.filter(l => 
            l.keywords.some(k => itemKeywords.includes(k))
        );
  ...
  private async runSchedulingPass(ctx: {
      ...
      const { userId, items, busySlots, phases, schedulingState, startTimeMs, habitDetails, externalScheduledIds } = ctx;

      busySlots.sort((a,b) => a.start - b.start);

      // We need to know which items are tasks vs habits for the loop
      const passTasks = items.filter(i => 'effortMinutes' in i); // Duck typing
      const passHabits = items.filter(i => 'weeklyGoalMinutes' in i);
      ...
      for (const phase of phases) {
        ...
        for (const task of passTasks) {
            ...
            if (task.projectId) {
                ...
                const hasEarlierPendingTask = passTasks.some(t => 
        ...
        for (const habit of passHabits) {
            ...
            rankedItems.push(this.calculateScore(habit, 'habit', phase.name, state, phaseStartMs, details));
        }
        ...
        while (rankedItems.length > 0) {
            ...
            if (chosenSlot) {
                const timeToBook = isPartial ? chosenSlot.duration : durationMs;
                ...
                if (item.type === 'task' && item.source.projectId) {
                    projectsScheduledThisPhase.add(item.source.projectId);
                }
  ...
  private calculateScore(
      ...
      const idealPhase = item.idealPhase;
      ...
      if (type === 'task') {
          const level = item.urgencyLevel || 'T6';
  ...
  private getUserConstraints(user: any, date: Date): {start: number, end: number}[] {
  ...
  private getWorkHours(user: any, date: Date): {start: number, end: number} | null {
  ...
  private addBusySlot(busySlots: {start:number, end:number}[], start: number, end: number) {
      busySlots.push({ start, end });
      busySlots.sort((a, b) => a.start - b.start);
  }

  private async createPlan(userId: string, item: RankedPlanItem, phaseName: string, start: Date, end: Date) {
        const dbStart = new Date(start.getTime() + 60000);
        const dbEnd = new Date(end.getTime() - 60000);
        const durationMins = Math.round((end.getTime() - start.getTime()) / 60000);
        
        console.log(`🤖 PLAN: ${item.type.toUpperCase()} | "${item.source.title}" | ${phaseName} | ${durationMins}m | IN-PROGRESS: ${item.isInProgress}`);
  ...
  private getFreeSlots(busySlots: {start:number, end:number}[], limitStart: number, limitEnd: number): {start: number, end: number, duration: number}[] {
